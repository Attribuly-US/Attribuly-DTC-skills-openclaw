...
**[2026-03-31] API キー設定フローのアップデート！**
- **設定の簡素化**: API キーの自動検出機能を追加しました。ユーザーがチャットに API キーを直接貼り付けるだけで、エージェントが自動的に環境変数を設定するようになりました。
- **インタラクションフローの改善**: `SKILL.md` を更新し、設定後にエージェントが繰り返しキーを要求するのを防ぎ、登録リンクの提供とキーの安全な取り扱いを保証するための厳格なネガティブプロンプト (DO NOTs) を追加しました。
- **多言語対応**: 設定手順とエージェントの応答を、英語、中国語、日本語のすべてで更新しました。
...
- **設定方法:** 最高レベルのセキュリティを確保するため、OpenClaw の **Agent Settings UI（環境変数/Secrets）** を通じて API キーを設定することをお勧めします。または、**チャットボックスに直接 API キーを貼り付ける**ことで、エージェントに自動的に設定させることもできます。
  - ⚠️ **セキュリティに関する注意事項:** チャット経由での設定を選択した場合、エージェントはローカルコマンド（`openclaw config set`）を実行して、環境設定にキーを永続化します。チャット履歴にキーが残るのを防ぐため、設定完了後に API キーが含まれるチャットメッセージを削除することを強くお勧めします。
