...
スキルフォルダが `openclaw-config/skills/` ディレクトリ（ローカルまたはクラウド）に正常に配置されたら、[SKILL\_REGISTRY.md](SKILL_REGISTRY.md) を確認して、各スキルを効果的に使用するための特定のトリガーと必要なコンテキストの詳細を参照してください。
